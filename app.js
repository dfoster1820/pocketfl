...
const ARCHETYPES = {
  QB: ["Gunslinger", "Game Manager", "Mobile", "Field General", "Dual Threat"],
  RB: ["Workhorse", "Speedster", "Power Back", "Receiving Back", "Bruiser"],
  WR: ["Deep Threat", "Possession", "Slot Specialist", "Route Technician", "Playmaker"],
  TE: ["Receiving Threat", "Blocking Specialist", "Move TE", "Hybrid"],
  OL: ["Pass Protector", "Road Grader", "Technician", "Anchor"],
  DL: ["Pass Rusher", "Run Stuffer", "Motor Guy", "Disruptor"],
  LB: ["Sideline to Sideline", "Thumper", "Coverage LB", "Blitzer"],
  CB: ["Shutdown", "Zone Specialist", "Ball Hawk", "Press Corner"],
  S: ["Ball Hawk", "Enforcer", "Coverage Safety", "Box Safety"],
  K: ["Dead Eye", "Big Leg", "Clutch", "Accurate"],
};
const CHARACTER_LEVELS = ["Low", "Med", "High"];
const MORALE_LEVELS = ["Unhappy", "Content", "Happy", "Thrilled"];
const TECHNICAL_SKILLS = {
  QB: ["Short Pass", "Medium Pass", "Deep Pass", "Throw on Run", "Ball Security", "Pocket Presence", "Play Action", "Scrambling", "Touch & Timing", "Progression Speed"],
  RB: ["Ball Security", "Contact Balance", "Elusiveness", "Trucking Power", "Pass Blocking", "Receiving", "Route Running", "Stiff Arm", "Change of Direction", "Hole Reading"],
  WR: ["Route Running", "Catching", "Contested Catch", "Release vs Press", "Yards After Catch", "Blocking", "Deep Threat", "Short Routes", "Body Control", "Hands"],
  TE: ["Route Running", "Catching", "Run Blocking", "Pass Blocking", "Yards After Catch", "Contested Catch", "Blocking Technique", "Hands", "In-Line Strength", "Seam Threat"],
  OL: ["Pass Blocking", "Run Blocking", "Pull Blocking", "Anchor", "Hand Technique", "Awareness", "Double Team", "Screen Blocking", "Snap Timing", "Footwork"],
  DL: ["Pass Rush", "Run Stopping", "Hand Technique", "Motor", "Block Shedding", "Gap Discipline", "Get Off", "Bull Rush", "Finesse Moves", "Pursuit"],
  LB: ["Run Stopping", "Man Coverage", "Zone Coverage", "Blitzing", "Tackling", "Read & React", "Block Shedding", "Sideline to Sideline", "Gap Control", "Play Recognition"],
  CB: ["Man Coverage", "Zone Coverage", "Press Coverage", "Ball Skills", "Tackling", "Backpedal", "Recovery Speed", "Route Recognition", "Jamming", "Long Speed"],
  S: ["Zone Coverage", "Man Coverage", "Run Support", "Tackling", "Ball Skills", "Range", "Box Presence", "Deep Coverage", "Play Recognition", "Communication"],
  K: ["Accuracy", "Leg Strength", "Clutch Kicking", "Kickoff Power", "Consistency", "Onside Kicks", "Wind Management", "Hang Time", "Field Goal Range", "Composure"],
};
...
function migrateSettings(s) {
  ...
  Object.values(s.players || {}).forEach(p => {
    ...
    if (p.archetype === undefined) p.archetype = choice(ARCHETYPES[p.pos] || ["Standard"]);
    if (p.morale === undefined) p.morale = "Content";
    if (p.tradeBlock === undefined) p.tradeBlock = false;
    if (!p.mental) p.mental = { intelligence: p.ovr || 60, vision: p.ovr || 60, decisions: p.ovr || 60, discipline: p.ovr || 60 };
    if (!p.character) p.character = { greed: "Med", loyalty: "Med", ambition: "Med", personality: "Med", workEthic: "Med" };
    if (!p.devFactors) p.devFactors = { coaching: 3, teamMentors: 3, personality: 3 };
    if (!p.awards) p.awards = { rings: 0, poty: 0, allPro: 0, confBowlApps: 0 };
    if (!p.injuryHistory) p.injuryHistory = [];
    if (!p.gameLog) p.gameLog = [];
    if (!p.technical) {
      p.technical = {};
      (TECHNICAL_SKILLS[p.pos] || []).forEach(skill => { p.technical[skill] = clamp((p.ovr || 60) + randInt(-10, 10), 40, 99); });
    }
  });
  (s.teams || []).forEach(t => {
    ["coach", "scout", "physio"].forEach(role => {
      if (!t.staff || !t.staff[role]) return;
      STAFF_STATS[role].forEach(([key]) => { if (t.staff[role][key] === undefined) t.staff[role][key] = randInt(1, 5); });
    });
  });
  return s;
}
...
function makePlayer(pos, opts = {}) {
  ...
  return {
    ...
    archetype: choice(ARCHETYPES[pos] || ["Standard"]),
    morale: "Content",
    ...
    mental: {
      intelligence: clamp(ovr + randInt(-10, 10), 40, 99),
      vision: clamp(ovr + randInt(-10, 10), 40, 99),
      decisions: clamp(ovr + randInt(-10, 10), 40, 99),
      discipline: clamp(ovr + randInt(-10, 10), 40, 99),
    },
    character: {
      greed: choice(CHARACTER_LEVELS), loyalty: choice(CHARACTER_LEVELS), ambition: choice(CHARACTER_LEVELS),
      personality: choice(CHARACTER_LEVELS), workEthic: choice(CHARACTER_LEVELS),
    },
    devFactors: { coaching: randInt(1, 5), teamMentors: randInt(1, 5), personality: randInt(1, 5) },
    technical: Object.fromEntries((TECHNICAL_SKILLS[pos] || []).map(skill => [skill, clamp(ovr + randInt(-10, 10), 40, 99)])),
    awards: { rings: 0, poty: 0, allPro: 0, confBowlApps: 0 },
    injuryHistory: [],
    gameLog: [],
...
const COACH_STATS = [
  ["dev", "Player Development"], ["gameday", "Gameday"], ["offenseScheme", "Offense Scheme"],
  ["defenseScheme", "Defense Scheme"], ["motivation", "Motivation"], ["discipline", "Discipline"],
  ["playcalling", "Playcalling"], ["adaptability", "Adaptability"],
];
const SCOUT_STATS = [
  ["evaluator", "Evaluator"], ["starSpotter", "Star Spotter"], ["regionalConnections", "Regional Connections"],
  ["filmStudy", "Film Study"], ["medicalRedFlags", "Medical Red Flags"], ["interviewProcess", "Interview Process"],
];
const PHYSIO_STATS = [
  ["prevention", "Prevention"], ["rehab", "Rehabilitation"], ["loadManagement", "Load Management"],
  ["nutritionProgram", "Nutrition Program"], ["returnTimelineAccuracy", "Return Timeline Accuracy"], ["sportsScienceIq", "Sports Science IQ"],
];
const STAFF_STATS = { coach: COACH_STATS, scout: SCOUT_STATS, physio: PHYSIO_STATS };

function randStaffStats(role) {
  const out = {};
  STAFF_STATS[role].forEach(([key]) => { out[key] = randInt(1, 5); });
  return out;
}
function makeStaff() {
  return {
...
function staffCost(member) {
  ...
  return Math.round(avg * 1.3 * 10) / 10;
}
  ...
  [homeTeam, awayTeam].forEach(team => {
    team.players.forEach(id => {
      ...
      if (chance > 0 && Math.random() < chance) {
        ...
        p.injuryHistory.push({ season: S.season, week: S.week, weeksOut: duration });
        if (p.injuryHistory.length > 30) p.injuryHistory.shift();
...
function statDelta(before, after) {
  ...
  for (const cat of Object.keys(after)) {
    if (typeof after[cat] === "number") { out[cat] = after[cat] - (before[cat] || 0); continue; }
    out[cat] = {};
    for (const k of Object.keys(after[cat])) {
      if (k === "long") out[cat][k] = after[cat][k] > (before[cat][k] || 0) ? after[cat][k] : 0;
      else out[cat][k] = (after[cat][k] || 0) - (before[cat][k] || 0);
    }
  }
  return out;
}
function logGameStats(team, oppId, isHome, before) {
  if (team.id !== S.userTeamId) return; // keep save size sane — CPU rosters still get season/career aggregates
  team.players.forEach(pid => {
    const p = S.players[pid]; const b = before[pid]; if (!b) return;
    p.gameLog.push({ season: S.season, week: S.week, opp: oppId, home: isHome, stats: statDelta(b, p.stats) });
    if (p.gameLog.length > 40) p.gameLog.shift();
  });
}
...
function simulateWeek() {
  ...
  for (const g of games) {
    ...
    logGameStats(home, away.id, true, before);
    logGameStats(away, home.id, false, before);
    results.push(g);
  }
...
function simPlayoffRound() {
  ...
  if (!didSomething && !po.championsBowl) {
    ...
    teamById(c1).players.forEach(pid => { if (S.players[pid]) S.players[pid].awards.confBowlApps += 1; });
    teamById(c2).players.forEach(pid => { if (S.players[pid]) S.players[pid].awards.confBowlApps += 1; });
    teamById(winner).players.forEach(pid => { if (S.players[pid]) S.players[pid].awards.rings += 1; });
...
const ALL_PRO_SLOTS = { QB: 1, RB: 2, WR: 3, TE: 1, DL: 2, LB: 2, CB: 2, S: 2, K: 1 };

function computeSeasonAwards() {
  const active = Object.values(S.players).filter(p => p.teamId && !p.retired && p.stats.gp > 0);
  if (!active.length) return;
  let potyWinner = null, potyScore = -Infinity;
  active.forEach(p => { const s = careerScore(p.stats); if (s > potyScore) { potyScore = s; potyWinner = p; } });
  if (potyWinner) { potyWinner.awards.poty += 1; addLog(`🏆 ${potyWinner.name} (${potyWinner.pos}) named Player of the Year.`); }

  Object.entries(ALL_PRO_SLOTS).forEach(([pos, slots]) => {
    active.filter(p => p.pos === pos).sort((a, b) => careerScore(b.stats) - careerScore(a.stats)).slice(0, slots)
      .forEach(p => { p.awards.allPro += 1; });
  });
}
...
function processOffseason() {
  ...
  for (const t of S.teams) {
    ...
    for (const pid of t.players) {
      ...
      const statsSnapshot = JSON.parse(JSON.stringify(p.stats));
      p.age += 1;
      ...
      p.seasonLog.push({ season: S.season, teamId: t.id, stats: statsSnapshot, ovr: p.ovr });
      addStatsInto(p.career, statsSnapshot);
      p.stats = freshStats();
...
function renderOfficeStaff(root) {
  ...
  const roles = [
    ["coach", "Head Coach", COACH_STATS],
    ["scout", "Chief Scout", SCOUT_STATS],
    ["physio", "Head Physio", PHYSIO_STATS],
  ];
...
function showHireModal(role, label) {
  ...
  const statKeys = STAFF_STATS[role];
  const node = el("div");
...
function showEditStaffModal(team, role, label, statDefs) {
  ...
  statDefs.forEach(([key, l]) => form.appendChild(field(l + " (1-5)", numInputWithBounds(draft[key], v => draft[key] = clamp(Math.round(v), 1, 5), { min: 1, max: 5, step: 1 }))));
...
function potentialLabel(pot) {
  if (pot >= 95) return "Generational Talent";
  if (pot >= 88) return "Elite Potential";
  if (pot >= 80) return "Pro Bowl Caliber";
  if (pot >= 70) return "Solid Starter";
  if (pot >= 60) return "Depth Piece";
...
function showEditPlayerModal(p, onSave, opts = {}) {
  ...
  const draft = {
    ...
    college: p.college, jerseyNumber: p.jerseyNumber, age: p.age, pos: p.pos, archetype: p.archetype, morale: p.morale,
  ...
  let tab = "personal";
  const node = el("div");
  node.appendChild(el("h2", { class: "section-title" }, isProspect ? "Edit College Prospect" : p.name));

  if (!isProspect) {
    const growth = p.seasonLog.length ? p.ovr - (p.seasonLog[p.seasonLog.length - 1].ovr ?? p.ovr) : 0;
    node.appendChild(el("div", { class: "stat-grid", style: "margin-bottom:14px" }, [
      statBox(p.archetype, "Archetype"),
      statBox(team ? teamName(team.id) : "Free Agent", "Team"),
      statBox(p.morale, "Morale"),
      statBox(`${p.ovr}${growth ? (growth > 0 ? ` (+${growth})` : ` (${growth})`) : ""}`, "Overall"),
    ]));
  }

  const tabsRow = el("div", { class: "chip-row" });
  const body = el("div");
  const tabList = isProspect
    ? [["personal", "Personal"], ["skills", "Skills"], ["contract", "Scouting"]]
    : [["personal", "Personal"], ["skills", "Skills"], ["develop", "Develop"], ["contract", "Contract"], ["games", "Games"], ["career", "Career"]];

  function renderTabs() {
    tabsRow.innerHTML = "";
    tabList.forEach(([k, l]) => tabsRow.appendChild(el("button", { class: `chip ${tab === k ? "active" : ""}`, onclick: () => { tab = k; renderTabs(); renderBody(); } }, l)));
  }
  function renderBody() {
    body.innerHTML = "";
    const fn = { personal: personalTab, skills: skillsTab, develop: developTab, contract: contractTab, games: gamesTab, career: careerTab }[tab];
    body.appendChild(fn());
  }
  ...
  function personalTab() {
    ...
    form.appendChild(field("Position", selectInput(POSITIONS, draft.pos, v => { draft.pos = v; renderBody(); })));
    form.appendChild(field("Archetype", textInput(draft.archetype, v => draft.archetype = v)));
    ...
    form.appendChild(field("Morale", selectInput(MORALE_LEVELS, draft.morale, v => draft.morale = v)));
  ...
  function skillsTab() {
    ...
    wrap.appendChild(el("p", { class: "field-label", style: "margin-bottom:6px" }, "Physical"));
    const form = el("div", { class: "form-grid" });
    ...
    wrap.appendChild(el("p", { class: "field-label", style: "margin:16px 0 6px" }, "Mental"));
    const mform = el("div", { class: "form-grid" });
    [["intelligence", "Intelligence"], ["vision", "Vision"], ["decisions", "Decisions"], ["discipline", "Discipline"]].forEach(([k, l]) => {
      mform.appendChild(field(l, numInputWithBounds(draft.mental[k], v => draft.mental[k] = clamp(Math.round(v), 40, 99), { min: 40, max: 99 })));
    });
    wrap.appendChild(mform);

    const techSkills = TECHNICAL_SKILLS[draft.pos] || [];
    if (techSkills.length) {
      wrap.appendChild(el("p", { class: "field-label", style: "margin:16px 0 6px" }, `Technical (${draft.pos})`));
      const tform = el("div", { class: "form-grid" });
      techSkills.forEach(skill => {
        if (draft.technical[skill] === undefined) draft.technical[skill] = clamp(draft.ovr, 40, 99);
        tform.appendChild(field(skill, numInputWithBounds(draft.technical[skill], v => draft.technical[skill] = clamp(Math.round(v), 40, 99), { min: 40, max: 99 })));
      });
      wrap.appendChild(tform);
    }
    wrap.appendChild(el("p", { class: "muted", style: "margin-top:12px" }, "Overall and Potential drive the simulation directly. Everything else on this tab is descriptive for now — except Injury Prone, which does affect how often this player gets hurt."));
    return wrap;
  }

  function developTab() {
    const wrap = el("div");
    const panel1 = el("div", { class: "panel" }, [el("h3", {}, "Rating Progression")]);
    if (p.seasonLog.length) {
      const chart = el("div", { style: "display:flex; align-items:flex-end; gap:8px; height:110px; padding-top:16px;" });
      p.seasonLog.slice(-8).forEach(log => {
        const ovr = log.ovr ?? 0;
        const h = clamp(((ovr - 40) / (99 - 40)) * 90, 4, 90);
        chart.appendChild(el("div", { style: "flex:1; display:flex; flex-direction:column; align-items:center; gap:4px;" }, [
          el("div", { class: "muted", style: "font-size:0.7rem" }, String(ovr)),
          el("div", { style: `width:100%; background:var(--blue); border-radius:3px 3px 0 0; height:${h}px;` }),
          el("div", { class: "muted", style: "font-size:0.65rem" }, `S${log.season}`),
        ]));
      });
      panel1.appendChild(chart);
    } else panel1.appendChild(el("p", { class: "muted" }, "No completed seasons yet."));
    ...
    wrap.appendChild(el("p", { class: "field-label", style: "margin-bottom:6px" }, "Development Factors"));
    const dform = el("div", { class: "form-grid" });
    [["coaching", "Coaching"], ["teamMentors", "Team Mentors"], ["personality", "Personality"]].forEach(([k, l]) => {
      dform.appendChild(field(l, numInputWithBounds(draft.devFactors[k], v => draft.devFactors[k] = clamp(Math.round(v), 1, 5), { min: 1, max: 5 })));
    });
    wrap.appendChild(dform);

    const injPanel = el("div", { class: "panel", style: "margin-top:16px" }, [el("h3", {}, "Injury History")]);
    if (!p.injuryHistory.length) injPanel.appendChild(el("p", { class: "muted" }, "No injuries on record."));
    p.injuryHistory.slice().reverse().forEach(inj => {
      injPanel.appendChild(el("div", { class: "list-row" }, [el("span", {}, `Season ${inj.season}, Week ${inj.week}`), el("span", { class: "muted" }, `Out ${inj.weeksOut} wk${inj.weeksOut > 1 ? "s" : ""}`)]));
    });
    wrap.appendChild(injPanel);
    return wrap;
  }
  ...
  function contractTab() {
    ...
    wrap.appendChild(el("p", { class: "field-label", style: "margin-bottom:6px" }, "Character"));
    const cform = el("div", { class: "form-grid" });
    [["greed", "Greed"], ["loyalty", "Loyalty"], ["ambition", "Ambition"], ["personality", "Personality"], ["workEthic", "Work Ethic"]].forEach(([k, l]) => {
      cform.appendChild(field(l, selectInput(CHARACTER_LEVELS, draft.character[k], v => draft.character[k] = v)));
    });
    wrap.appendChild(cform);

    wrap.appendChild(el("p", { class: "field-label", style: "margin:16px 0 6px" }, "Contract"));
    const form = el("div", { class: "form-grid" });
    ...
    if (team) {
      const tradeRow = el("div", { class: "row-gap", style: "margin-top:14px; align-items:center;" }, [
        el("span", { class: "field-label" }, "Trade Block"),
        el("button", { class: `chip ${draft.tradeBlock ? "active" : ""}`, onclick: () => { draft.tradeBlock = !draft.tradeBlock; renderBody(); } }, draft.tradeBlock ? "Available" : "Not Available"),
      ]);
      wrap.appendChild(tradeRow);
  ...
  function gamesTab() {
    const wrap = el("div");
    const chips = el("div", { class: "chip-row" });
    chips.appendChild(el("button", { class: `chip ${gamesTab._mode !== "career" ? "active" : ""}`, onclick: () => { gamesTab._mode = "season"; renderBody(); } }, "This Season"));
    chips.appendChild(el("button", { class: `chip ${gamesTab._mode === "career" ? "active" : ""}`, onclick: () => { gamesTab._mode = "career"; renderBody(); } }, "Career"));
    wrap.appendChild(chips);

    const rows = gamesTab._mode === "career" ? p.gameLog : p.gameLog.filter(g => g.season === S.season);
    const panel = el("div", { class: "panel" });
    if (!rows.length) {
      const msg = team && team.id !== S.userTeamId
        ? "Detailed game-by-game logs are only tracked for your own roster (to keep save files a reasonable size). This player's season and career totals are still tracked normally."
        : "No games played yet.";
  ...
  function careerTab() {
    ...
    wrap.appendChild(el("div", { class: "stat-grid", style: "margin-bottom:14px" }, [
      statBox(p.awards.rings, "Rings"), statBox(p.awards.poty, "POTY"), statBox(p.awards.allPro, "All-Pro"), statBox(p.awards.confBowlApps, "Conf Bowl"),
    ]));
    const statsPanel = el("div", { class: "panel" }, [el("h3", {}, "Career Stats"), el("div", { class: "stat-grid" }, [
      statBox(p.career.gp, "Games"), statBox(String(playerValue(p) | 0), "Value Score"),
    ]), el("p", { class: "muted", style: "margin-top:10px" }, statLine(p.career, p.pos))]);
    wrap.appendChild(statsPanel);

    const draftPanel = el("div", { class: "panel" }, [el("h3", {}, "Draft Info"), el("div", { class: "stat-grid" }, [
      statBox(team ? teamName(team.id).split(" ").pop() : "—", "Picked By"),
      statBox(p.draftPick ? `#${p.draftPick}` : "UDFA", "Pick"),
      statBox(p.draftSeason || "—", "Year"),
      statBox(p.college, "College"),
    ])]);
    wrap.appendChild(draftPanel);

    const score = careerScore(p.career);
    const pct = clamp(Math.round((score / HOF_THRESHOLD) * 100), 0, 100);
    const hofPanel = el("div", { class: "panel" }, [
      el("h3", {}, "Hall of Fame Progress"),
      el("div", { style: "background:var(--paper-2); border-radius:20px; height:10px; overflow:hidden; margin:8px 0;" }, [
        el("div", { style: `background:var(--blue); height:100%; width:${pct}%;` }),
      ]),
      el("p", { class: "muted" }, `${pct}% of the way to an elite career (score ${Math.round(score)} / ${HOF_THRESHOLD}).`),
    ]);
    wrap.appendChild(hofPanel);
    return wrap;
  }
  ...
  node.appendChild(el("div", { class: "row-gap", style: "margin-top:14px" }, [
    el("button", { class: "btn btn-primary", onclick: () => {
      ...
      Object.assign(p, {
        ...
        archetype: draft.archetype, morale: draft.morale,
        ...
        mental: draft.mental, technical: draft.technical,
      });
      if (isProspect) { p._scoutLow = draft.scoutLow; p._scoutHigh = draft.scoutHigh; }
      else {
        p.salary = draft.salary; p.bonus = draft.bonus; p.contractYears = draft.contractYears; p.weightContract = draft.weightContract; p.salaryOverride = true;
        p.character = draft.character; p.devFactors = draft.devFactors; p.tradeBlock = draft.tradeBlock;
      }
